...
for(j in 1:permutations){
     ...
     auc.df[nrow(auc.df) + 1,] = c(IDs[i], auc(rf_roc), TRUE, "pre")

     if (batch == 1){df <- data.frame(Study_ID = IDs[i], DEBIAS = "pre", Permutation = j, sensitivities = rf_roc$sensitivities, specificities = rf_roc$specificities)}else{
       df <- data.frame(Study_ID = IDs[i], DEBIAS = "pre", Permutation = (j+(permutations*(batch-1))), sensitivities = rf_roc$sensitivities, specificities = rf_roc$specificities)
     }
...
if (batch == 1){
  ...
  auc.df[nrow(auc.df) + 1,] = c(IDs[i], auc(rf_roc), FALSE, "pre")
  auc.df$AUC <- as.numeric(auc.df$AUC) # convert AUC in auc.df to numeric
  
  
  ## add ROC to ROC df
  df <- data.frame(Study_ID = IDs[i], DEBIAS = "pre", Permutation = 0, sensitivities = rf_roc$sensitivities, specificities = rf_roc$specificities)
  roc.df <- rbind(roc.df, df)
