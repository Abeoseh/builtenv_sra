...
for file in files:
    ...
    with open(f"../{name}/manifest.tsv", "r") as file:
        for line in file:
            line = line.strip("\n")
            line = line.split("\t")
    ...
    rename = rename.pop("sample-id")

    for i in range(len(list(df.columns))):
        rename[list(df.columns)[i]] = list(df.columns)[i]+"_"+filename
