...
#### make roc df ####
roc_files = list.files(paste("./output/", input,"/CSVs",sep=""), "*_ROCs", full.names = TRUE)
...
for (ID in IDs){
  ...
  timepoints = unique(current_auc$DEBIAS)
  # print(timepoints)
  for (timepoint in timepoints){
    # print(timepoint)
    current_auc_df <- filter(current_auc, DEBIAS == !!timepoint)
    ...
    current_pval_df <- data.frame(Study_ID = ID, pval = for.pval, DEBIAS = timepoint)
...
#### make ROC curves ####
print("starting ROC curves")
for (ID in IDs){
  plot_title = study$Author[study$ID == ID]
  current_roc <- filter(roc, Study_ID == !!ID)
  
  timepoints = unique(current_roc$DEBIAS)
  for (timepoint in timepoints){
    png(paste("./output/",input,"/plots/",timepoint,"_DEBIAS-M_RF_lognorm_ROC_", ID, ".png", sep=""))#, height = 24, width = 24)
    ...
    title(paste("Leave out: ", plot_title, sep=""), cex.main=1.5)
    ...
    for(permutation in 1:length(permutations)){
      permutation_roc <- filter(current_roc, Permutation == permutation & DEBIAS == !!timepoint)
      # print("done 3")
    ...
    ## plot actual data
    actual_data_roc <- filter(current_roc, Permutation == 0 & DEBIAS == !!timepoint)
    # print("done 4")
    lines(actual_data_roc$specificities, actual_data_roc$sensitivities, col="red", lwd = 3)
    
    
    ## add permutations
    for.pval = pval.df$pval[pval.df$Study_ID == ID & pval.df$DEBIAS == timepoint] %>% signif(3)
    # print("done 5")
    for.auc = auc.df$AUC[auc.df$Study_ID == ID & auc.df$Permutation == FALSE & auc.df$DEBIAS == timepoint] %>% round(3)
    # print("done 6")
